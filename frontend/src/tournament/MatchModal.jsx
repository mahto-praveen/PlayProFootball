...
const MatchModal = ({ match, tournamentId, onClose }) => {
  ...
  const saveMatch = async () => {
    if (match?.id) {
      ...
      await axios.put(`/api/matches/${match.id}`, updatePayload);
    } else {
      // Create new match
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-6 rounded w-[400px]">
        ...
        <input className="border p-2 w-full mb-2" type="datetime-local" value={formData.scheduledAt} onChange={e => setFormData({ ...formData, scheduledAt: e.target.value })} />
        ...
        <select className="border p-2 w-full mb-4" value={formData.status} onChange={e => setFormData({ ...formData, status: e.target.value })}>
